...
The dataset utilised for the below analysis consists of a total of 34,552 nodes and 5,464,092 direct edges (shipper to receiver). It is a directed multi-graph, hence multiple edges between the same two nodes are possible. The full details can be found on: https://vast-challenge.github.io/2023/MC2.html
...
#| code-fold: true
#| code-summary: "Show code"
na_values <- colMeans(is.na(MC2_edges)) * 100

for (col in names(na_values)) {
  cat("NA % in", col, ":", na_values[[col]], "%\n")
}
```
...
### 4.5 Top 8 Sources across the Years by varying metrics
Within this section, we take a look a the top 8 sources across the years by weightkg and value of goods usd. We looped over the years to calculate the top 8 sources by the relevant metrics for each year, before leveraging on plot_ly() to create a line chart to visualise the trends over the years.
...
### 4.6 Top 8 Targets across the Years by varying metrics
Within this section, we take a look a the top 8 targets across the years by weightkg and value of goods usd. We looped over the years to calculate the top 8 targets by the relevant metrics for each year, before leveraging on plot_ly() to create a line chart to visualise the trends over the years.
...
The below shows the code for plotting a group components plot which primarily groups by connected components.
...
Finally, the below shows the plot for leading eigen cluster which groups nodes based on the leading eigenvector of the modularity matrix.
...
## 6.2 Out-Degree Centrality by Years
...
MC2_edges_weight_2029_outdegree <- MC2_edges %>%
  filter(year == 2029) %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
...
id1_weight_2029_outdegree <- MC2_edges_weight_2029_outdegree %>%
  select(source) %>%
  rename(id = source)
id2_weight_2029_outdegree <- MC2_edges_weight_2029_outdegree %>%
  select(target) %>%
  rename(id = target)

MC2_nodes_extracted_weight_2029_outdegree <- rbind(id1_weight_2029_outdegree, id2_weight_2029_outdegree) %>% 
  distinct()

MC2_nodes_extracted_weight_2029_outdegree <- left_join(MC2_nodes_extracted_weight_2029_outdegree, MC2_nodes_cleaned, by = "id")

# Create the graph
MC2_graph_weights_2029_outdegree <- tbl_graph(nodes=MC2_nodes_extracted_weight_2029_outdegree,
                                             edges = MC2_edges_weight_2029_outdegree, 
                                             directed = TRUE)

# Calculate degree centrality & define threshold to assign nodes to either Central Carrier or Fishing Vessel
outdegree_centrality_2029 <- degree(MC2_graph_weights_2029_outdegree, mode = "out")
outdegree_threshold_2029 <- 2

# Set size to degree centrality
V(MC2_graph_weights_2029_outdegree)$size <- outdegree_centrality_2029

# Assign categories based on centrality measures
outdegree_categories_2029 <- ifelse(outdegree_centrality_2029 > outdegree_threshold_2029, "Central Carrier", "Fishing Vessel")

# Store category information in MC2_nodes_extracted_weight_2029
MC2_nodes_extracted_weight_2029_outdegree$category <- outdegree_categories_2029

# Visualize the filtered graph using ggraph
ggraph(MC2_graph_weights_2029_outdegree, layout = "fr") +
  geom_edge_link(arrow = arrow(length = unit(0.02, "npc"))) +
  geom_node_point(aes(color = outdegree_categories_2029, size = size)) +
  scale_color_manual(values = c("Central Carrier" = "red", "Fishing Vessel" = "grey")) +
  scale_size_continuous(range = c(1, 2)) +
  theme(legend.position = "right",
        panel.grid.major = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "2029 Graph based on Out-Degree Centrality")
```
...
MC2_edges_weight_2030_outdegree <- MC2_edges %>%
  filter(year == 2030) %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
...
id1_weight_2030_outdegree <- MC2_edges_weight_2030_outdegree %>%
  select(source) %>%
  rename(id = source)
id2_weight_2030_outdegree <- MC2_edges_weight_2030_outdegree %>%
  select(target) %>%
  rename(id = target)

MC2_nodes_extracted_weight_2030_outdegree <- rbind(id1_weight_2030_outdegree, id2_weight_2030_outdegree) %>% 
  distinct()

MC2_nodes_extracted_weight_2030_outdegree <- left_join(MC2_nodes_extracted_weight_2030_outdegree, MC2_nodes_cleaned, by = "id")

# Create the graph
MC2_graph_weights_2030_outdegree <- tbl_graph(nodes=MC2_nodes_extracted_weight_2030_outdegree,
                                             edges = MC2_edges_weight_2030_outdegree, 
                                             directed = TRUE)

# Calculate degree centrality & define threshold to assign nodes to either Central Carrier or Fishing Vessel
outdegree_centrality_2030 <- degree(MC2_graph_weights_2030_outdegree, mode = "out")
outdegree_threshold_2030 <- 2

# Set size to degree centrality
V(MC2_graph_weights_2030_outdegree)$size <- outdegree_centrality_2030

# Assign categories based on centrality measures
outdegree_categories_2030 <- ifelse(outdegree_centrality_2030 > outdegree_threshold_2030, "Central Carrier", "Fishing Vessel")

# Store category information in MC2_nodes_extracted_weight_2030
MC2_nodes_extracted_weight_2030_outdegree$category <- outdegree_categories_2030

# Visualize the filtered graph using ggraph
ggraph(MC2_graph_weights_2030_outdegree, layout = "fr") +
  geom_edge_link(arrow = arrow(length = unit(0.02, "npc"))) +
  geom_node_point(aes(color = outdegree_categories_2030, size = size)) +
  scale_color_manual(values = c("Central Carrier" = "red", "Fishing Vessel" = "grey")) +
  scale_size_continuous(range = c(1, 2)) +
  theme(legend.position = "right",
        panel.grid.major = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "2030 Graph based on Out-Degree Centrality")
```
...
MC2_edges_weight_2031_outdegree <- MC2_edges %>%
  filter(year == 2031) %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
...
id1_weight_2031_outdegree <- MC2_edges_weight_2031_outdegree %>%
  select(source) %>%
  rename(id = source)
id2_weight_2031_outdegree <- MC2_edges_weight_2031_outdegree %>%
  select(target) %>%
  rename(id = target)

MC2_nodes_extracted_weight_2031_outdegree <- rbind(id1_weight_2031_outdegree, id2_weight_2031_outdegree) %>% 
  distinct()

MC2_nodes_extracted_weight_2031_outdegree <- left_join(MC2_nodes_extracted_weight_2031_outdegree, MC2_nodes_cleaned, by = "id")

# Create the graph
MC2_graph_weights_2031_outdegree <- tbl_graph(nodes=MC2_nodes_extracted_weight_2031_outdegree,
                                             edges = MC2_edges_weight_2031_outdegree, 
                                             directed = TRUE)

# Calculate degree centrality & define threshold to assign nodes to either Central Carrier or Fishing Vessel
outdegree_centrality_2031 <- degree(MC2_graph_weights_2031_outdegree, mode = "out")
outdegree_threshold_2031 <- 2

# Set size to degree centrality
V(MC2_graph_weights_2031_outdegree)$size <- outdegree_centrality_2031

# Assign categories based on centrality measures
outdegree_categories_2031 <- ifelse(outdegree_centrality_2031 > outdegree_threshold_2031, "Central Carrier", "Fishing Vessel")

# Store category information in MC2_nodes_extracted_weight_2031
MC2_nodes_extracted_weight_2031_outdegree$category <- outdegree_categories_2031

# Visualize the filtered graph using ggraph
ggraph(MC2_graph_weights_2031_outdegree, layout = "fr") +
  geom_edge_link(arrow = arrow(length = unit(0.02, "npc"))) +
  geom_node_point(aes(color = outdegree_categories_2031, size = size)) +
  scale_color_manual(values = c("Central Carrier" = "red", "Fishing Vessel" = "grey")) +
  scale_size_continuous(range = c(1, 2)) +
  theme(legend.position = "right",
        panel.grid.major = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "2031 Graph based on Out-Degree Centrality")
```
...
MC2_edges_weight_2032_outdegree <- MC2_edges %>%
  filter(year == 2032) %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
...
id1_weight_2032_outdegree <- MC2_edges_weight_2032_outdegree %>%
  select(source) %>%
  rename(id = source)
id2_weight_2032_outdegree <- MC2_edges_weight_2032_outdegree %>%
  select(target) %>%
  rename(id = target)

MC2_nodes_extracted_weight_2032_outdegree <- rbind(id1_weight_2032_outdegree, id2_weight_2032_outdegree) %>% 
  distinct()

MC2_nodes_extracted_weight_2032_outdegree <- left_join(MC2_nodes_extracted_weight_2032_outdegree, MC2_nodes_cleaned, by = "id")

# Create the graph
MC2_graph_weights_2032_outdegree <- tbl_graph(nodes=MC2_nodes_extracted_weight_2032_outdegree,
                                             edges = MC2_edges_weight_2032_outdegree, 
                                             directed = TRUE)

# Calculate degree centrality & define threshold to assign nodes to either Central Carrier or Fishing Vessel
outdegree_centrality_2032 <- degree(MC2_graph_weights_2032_outdegree, mode = "out")
outdegree_threshold_2032 <- 2

# Set size to degree centrality
V(MC2_graph_weights_2032_outdegree)$size <- outdegree_centrality_2032

# Assign categories based on centrality measures
outdegree_categories_2032 <- ifelse(outdegree_centrality_2032 > outdegree_threshold_2032, "Central Carrier", "Fishing Vessel")

# Store category information in MC2_nodes_extracted_weight_2032
MC2_nodes_extracted_weight_2032_outdegree$category <- outdegree_categories_2032

# Visualize the filtered graph using ggraph
ggraph(MC2_graph_weights_2032_outdegree, layout = "fr") +
  geom_edge_link(arrow = arrow(length = unit(0.02, "npc"))) +
  geom_node_point(aes(color = outdegree_categories_2032, size = size)) +
  scale_color_manual(values = c("Central Carrier" = "red", "Fishing Vessel" = "grey")) +
  scale_size_continuous(range = c(1, 2)) +
  theme(legend.position = "right",
        panel.grid.major = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "2032 Graph based on Out-Degree Centrality")
```
...
MC2_edges_weight_2033_outdegree <- MC2_edges %>%
  filter(year == 2033) %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
...
id1_weight_2033_outdegree <- MC2_edges_weight_2033_outdegree %>%
  select(source) %>%
  rename(id = source)
id2_weight_2033_outdegree <- MC2_edges_weight_2033_outdegree %>%
  select(target) %>%
  rename(id = target)

MC2_nodes_extracted_weight_2033_outdegree <- rbind(id1_weight_2033_outdegree, id2_weight_2033_outdegree) %>% 
  distinct()

MC2_nodes_extracted_weight_2033_outdegree <- left_join(MC2_nodes_extracted_weight_2033_outdegree, MC2_nodes_cleaned, by = "id")

# Create the graph
MC2_graph_weights_2033_outdegree <- tbl_graph(nodes=MC2_nodes_extracted_weight_2033_outdegree,
                                             edges = MC2_edges_weight_2033_outdegree, 
                                             directed = TRUE)

# Calculate degree centrality & define threshold to assign nodes to either Central Carrier or Fishing Vessel
outdegree_centrality_2033 <- degree(MC2_graph_weights_2033_outdegree, mode = "out")
outdegree_threshold_2033 <- 2

# Set size to degree centrality
V(MC2_graph_weights_2033_outdegree)$size <- outdegree_centrality_2033

# Assign categories based on centrality measures
outdegree_categories_2033 <- ifelse(outdegree_centrality_2033 > outdegree_threshold_2033, "Central Carrier", "Fishing Vessel")

# Store category information in MC2_nodes_extracted_weight_2033
MC2_nodes_extracted_weight_2033_outdegree$category <- outdegree_categories_2033

# Visualize the filtered graph using ggraph
ggraph(MC2_graph_weights_2033_outdegree, layout = "fr") +
  geom_edge_link(arrow = arrow(length = unit(0.02, "npc"))) +
  geom_node_point(aes(color = outdegree_categories_2033, size = size)) +
  scale_color_manual(values = c("Central Carrier" = "red", "Fishing Vessel" = "grey")) +
  scale_size_continuous(range = c(1, 2)) +
  theme(legend.position = "right",
        panel.grid.major = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "2033 Graph based on Out-Degree Centrality")
```
...
MC2_edges_weight_2034_outdegree <- MC2_edges %>%
  filter(year == 2034) %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
...
id1_weight_2034_outdegree <- MC2_edges_weight_2034_outdegree %>%
  select(source) %>%
  rename(id = source)
id2_weight_2034_outdegree <- MC2_edges_weight_2034_outdegree %>%
  select(target) %>%
  rename(id = target)

MC2_nodes_extracted_weight_2034_outdegree <- rbind(id1_weight_2034_outdegree, id2_weight_2034_outdegree) %>% 
  distinct()

MC2_nodes_extracted_weight_2034_outdegree <- left_join(MC2_nodes_extracted_weight_2034_outdegree, MC2_nodes_cleaned, by = "id")

# Create the graph
MC2_graph_weights_2034_outdegree <- tbl_graph(nodes=MC2_nodes_extracted_weight_2034_outdegree,
                                             edges = MC2_edges_weight_2034_outdegree, 
                                             directed = TRUE)

# Calculate degree centrality & define threshold to assign nodes to either Central Carrier or Fishing Vessel
outdegree_centrality_2034 <- degree(MC2_graph_weights_2034_outdegree, mode = "out")
outdegree_threshold_2034 <- 2

# Set size to degree centrality
V(MC2_graph_weights_2034_outdegree)$size <- outdegree_centrality_2034

# Assign categories based on centrality measures
outdegree_categories_2034 <- ifelse(outdegree_centrality_2034 > outdegree_threshold_2034, "Central Carrier", "Fishing Vessel")

# Store category information in MC2_nodes_extracted_weight_2034
MC2_nodes_extracted_weight_2034_outdegree$category <- outdegree_categories_2034

# Visualize the filtered graph using ggraph
ggraph(MC2_graph_weights_2034_outdegree, layout = "fr") +
  geom_edge_link(arrow = arrow(length = unit(0.02, "npc"))) +
  geom_node_point(aes(color = outdegree_categories_2034, size = size)) +
  scale_color_manual(values = c("Central Carrier" = "red", "Fishing Vessel" = "grey")) +
  scale_size_continuous(range = c(1, 2)) +
  theme(legend.position = "right",
        panel.grid.major = element_blank(),
        ...
        panel.background = element_rect(fill = "white"),
        plot.title = element_text(hjust = 0.5)) +
  labs(title = "2034 Graph based on Out-Degree Centrality")
```
...
## 6.3 Eigenvector Centrality by Years
Within this section, we will leverage on eigenvector centrality and ggraph to plot a static network graph. Eigenvector Centrality helps to detect notes that are has a more transitive influence. A node who has a higher eigenvector score implies that it is connected to many nodes who also have high scores. Note that the dataset is furthered focused into a smaller subset in the below section as we are filtering for weights which are >= quantile 0.8. 

Within the code outlined in Code Chunk below, we have also defined a node to be a "Central Carrier" if it's above a threshold currently defined at 0.3. If otherwise, then the node will be defined as "Fishing Vessel". This value was arrived after a few trial and errors before deciding that this was an optimal value. This helps us to detect transshipments which are frequent perpetrators of IUU fishing occuring out at sea.
...
## Mini-Case 2 Challenge Writeup & 4 Images
For the purpose of mini-case 2 challenge writeup question 4, it is as per below bullet points. The supporting images are found in section 7.1 - 7.4.

- IUU fishing occurrences typically occurs in the high seas. Given that transshipments are a huge potential for IUU fishing trades to occur as fishing vessels can obscure the origins of their catches that might have been caught illegally. The focus of this visualisation targets the transshipments between central carriers and fishing vessels. The visualisation outlined Section 7 serves to put forth the basis to enable the identification of companies that fit patterns of illegal fishing. 

- Leveraging on Section 7.2 Interactive Visual, “Turkish Mussels LLC and” company is one example of a company that fit a pattern of illegal fishing. It was active in 2028 with several in-degree associations with three main nodes: 1) Arena del Sol Pic, 2) Mar del Placer CJSC Transport and 3) Saltwater Sirens Limited Liability Company Marine conservation. However, “Turkish Mussels LLC and” company seems to have closed down thereafter. It appears that these three nodes are going to “Turkish Mussels LLC and” to obtain illegal catches. Arena del Sol Pic has a high outdegree centrality as it goes out to several nodes such as “Saltwater Supreme OAO Forwarding” and “Selous Game Reserve S.A. de C.V” during 2029 for example to get fish supplies. Notably, Arena del Sol Pic is still in the business even in 2034, reaching out to nodes such as “Mar del Este CJSC” which has a high indegree centrality. This seems to imply that Arena del Sol Pic is the end user of the fishing product it gets from its supplier (ie implying that it brings the fishes directly out to the market for sale).
...
### 7.4 Ego Network
Here, we explore the Ego network graph for a specific node: "Turkish Mussels LLC and" which was only active in 2028.
...
MC2_edges_weight_ego <- MC2_edges %>%
  filter(source == "Turkish Mussels LLC and" | target == "Turkish Mussels LLC and") %>%
  filter(hscode == "306170" | hscode == "304620" | hscode == "160414") %>%
  group_by(source, target, hscode, year) %>%
  summarise(weights = n(), .groups = "drop") %>%
  filter(source != target) %>%
