...
```{r}
pacman::p_load(plotly, lubridate, tidyverse, psych, ggridges, gganimate, ggrepel, tidyr)
```
...
3.  The **`pivot_wider()`** function is used to pivot the data into one column for each category and the corresponding **`amount`** values in each cell. values_fill is set as 0 to cater for scenario where categories may have a missing value.
...
From the second Ridgeline graph, we want to gain a closer insight into an individuals' **`Spending`**, **`Savings`** and **`Wage`**.We can observe that an individuals' **`Spending`** is largely center around a common range despite the huge variation in **`Wage`**. **`Savings`** and **`Wage`** tend to have a wider spread as compared to **`Spending`**.
...
Next, we carve out a dataframe called **`financialjournal_avg`** which contains the average values for **`Education`**, **`Food`**, **`Recreation`**, **`Shelter`**, **`Wage`**, **`Savings`**, **`Spending`**.
...
We sliced the age groups into three main buckets, 1) 18 - 30 years old, 2) 31 - 55 years old and 3) \>55 years old.
...
We delve deeper into the age group of the participants to better understand the demographics on hand.
...
We delve deeper into the education background level of the participants to better understand the demographics on hand.
...
In this section, we are keen to explore if there is a correlation between wage and spending. Interestingly, there is a weak correlation at 0.14 between wage and spending.
...
#| fig-width: 6
#| fig-height: 6
cor_val_wagespending <- cor(merged_data$avg_wage, merged_data$avg_spending)

ggplot(merged_data, aes(x = avg_wage, y = avg_spending)) +
  geom_point() +
  labs(x = "Wage", y = "Spending") +
  geom_text(x = max(merged_data$avg_wage), y = max(merged_data$avg_spending), 
            label = paste("Correlation:", round(cor(merged_data$avg_wage, merged_data$avg_spending), 2)),
...
#| fig-width: 6
#| fig-height: 6
merged_data$joviality <- as.numeric(merged_data$joviality)
cor_val_wagejoviality <- cor(merged_data$avg_wage, merged_data$joviality)

ggplot(merged_data, aes(x = avg_wage, y = joviality)) +
  geom_point() +
  labs(x = "Wage", y = "Joviality") +
  geom_text(x = max(merged_data$avg_wage), y = max(merged_data$joviality), 
            label = paste("Correlation:", round(cor(merged_data$avg_wage, merged_data$joviality), 2)),
            hjust = 1, vjust = 1, size = 4) +
  theme_classic()
```
