...
Note that st_read and read_sf provides the same result. If it's a shp file, then we need to define the dsn which is the destination folder, i.e, where the geospatial data is held at. Additionally, we need to inform R of the specific layer.

Within our folder, there is a collection of file ending with .dbf, .shp. shx, Therefore, we need to tell R the layer we are using, we do not need to tell it which extension as it will be able to pick up all of it.

Upon running the code below, we will see that there is one field called "Geometry". Note that mpsz is a sf data model in a dataframe format. "Geometry" field will provide the coordinate pairs. If it's a polygon, it will provide the coordinate pair of the polygon.
...
popdata2020 <- popdata %>%
  filter(Time == 2020) %>%
  group_by(PA, SZ, AG) %>%  #aggregate data based on planning area, subzone and agegroup
  summarise(`POP` = sum(`Pop`)) %>% #sum population by planning area, subzone and agegroup
  ungroup()%>%
...
GGplot2 is generic visualisation and does not have mapping such as classification, hence we are looking to use tmap instead. tmap is specially designed for thematic mapping. tmap is written based on grammar of the graphics, hence the syntax will be very similar to ggplot2. tmap also have facets similar to ggplot2. 

Additionally, tmap has static and interactive mode. 

Note that qtm refers to "quick tmap".
...
The graph below is in metres, hence we observe that the x and y axis contains big values.

```{r}
tm_shape(mpsz_pop2020)+
  tm_fill("DEPENDENCY", #used to create/draw the tmap polygon shape layer
          style = "quantile", #classification method
          palette = "Blues",
          title = "Dependency ratio") + #n refers to number of cluster that we want to use
  tm_layout(main.title = "Distribution of Dependency Ratio by planning subzone",
            ...
            legend.width = 0.35,
            frame = TRUE) +
  tm_borders(alpha = 0.5) + #border of the polygon, alpha defines the intensity
...
A histogram was also added into the map below via legend.hist.z().
...
In this example, small multiple choropleth maps are created by defining ncols in tm_fill(), thereby showing two separate choropleth map (one for "YOUNG", while another for "AGED")
...
tm_shape(mpsz_pop2020) +
  ...
  tm_facets(by="REGION_N", 
            free.coords=TRUE, #free.coords will create a facet based on the extent of the map field for each facet to avoid empty spaces.
            drop.shapes=TRUE) +
